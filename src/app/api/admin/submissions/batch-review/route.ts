...
export async function POST(request: Request) {
  ...
  if (!ids?.length || !['approve', 'reject'].includes(action)) {
    return Response.json({ error: { code: 'BAD_REQUEST', message: 'ids 和 action 必填' } }, { status: 400 });
  }
  ...
  for (const id of ids as string[]) {
    const sub = await db.all<SubmissionRow>(`SELECT * FROM submissions WHERE id = ? AND status IN ('queued', 'auto_flagged', 'in_review')`, [id]);
    if (!sub.results?.length) continue;
  ...
  return Response.json({ data: { processed: successCount, total: ids.length } });
}
