...
async function main() {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, name, url, normalized_domain, icon_path, icon_source_url, icon_mime_type, icon_fetch_status
       FROM websites
    ...
    await runWithConcurrency(rows, options.concurrency, async (site, index) => {
      try {
        ...
        failedCount += 1;
        console.log(`[icons:fetch] miss ${index + 1}/${rows.length} ${site.normalized_domain} ${result.reason}`);
      } catch (error) {
        await updateIconRecord(pool, site.id, {
          iconPath: null,
          iconSourceUrl: null,
