...
function upsertWebsiteFromRecord(state, record, source) {
  ...
  state.websiteRows.push([
    ...
    record.icon_path ? String(record.icon_path).trim().slice(0, 512) : null,
    record.icon_source_url ? String(record.icon_source_url).trim().slice(0, 2048) : null,
    record.icon_mime_type ? String(record.icon_mime_type).trim().slice(0, 120) : null,
    record.icon_fetch_status ? String(record.icon_fetch_status).trim().slice(0, 20) : null,
    record.icon_fetched_at ? String(record.icon_fetched_at).trim().slice(0, 32) : null,
...
async function main() {
  ...
  files.push(...writeChunk('websites', [
    ...
    update: [
      ...
      '`icon_source_url`=COALESCE(`icon_source_url`, VALUES(`icon_source_url`))',
      '`icon_mime_type`=COALESCE(`icon_mime_type`, VALUES(`icon_mime_type`))',
      '`icon_fetch_status`=COALESCE(`icon_fetch_status`, VALUES(`icon_fetch_status`))',
      '`icon_fetched_at`=COALESCE(`icon_fetched_at`, VALUES(`icon_fetched_at`))',
